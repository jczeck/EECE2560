...
//Finds the matches in the word list or hash table and grid
template <typename T>
void FindMatches(T& list, Grid& grid) {
    //Declare Variables
...
void Search(int algo) {
    ...
    //Create a hash table for use if hash table is chosen
    HashTable<string> table = HashTable<string>();
    ...
    case(4) :
        ...
        startTime = clock();
        //Create a hash table and copy all of the words into the list
        for (string word : list.GetWordList()) {
            table.AddItem(word);
        }
        endTimeSort = clock();
        FindMatches(table, grid);
        break;
...
int main(){   
    ...
    try {
        ...
        while (sortSelection != 5) {
            ...
            cout << "Enter 4 for Hash Table" << endl;
