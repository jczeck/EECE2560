...
//Sorts the words using heap sort
void WordList::HeapSort() {
    Heap<string> heap = Heap<string>(); //Create a string heap
    heap.InitializeMaxHeap(wordList); //Initialize a max heap with the wordList
    heap.Heapsort(wordList.size()); //Heapsort the heap
    //Read out the sorted heap into wordList
    for (int i = 0; i < wordList.size(); i++) {
        wordList.at(i) = heap.GetItem(i + 1);
    }
} //End Heapsort
