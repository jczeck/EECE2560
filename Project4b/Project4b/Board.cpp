...
*  Jenna Czeck
*  EECE 2560 Project 4
*  Implimentation of the board class
*/
...
ValueType Board::GetCell(int i, int j) const {
    //Return value if within size range, else throw error
    if (i >= 1 && i <= boardSize && j >= 1 && j <= boardSize)
        return value[i][j];
    else
...
bool Board::CausesConflict(int i, int j, ValueType val) const {
    //Throw error if out of board range
    if (i < 1 || i > boardSize || j < 1 || j > boardSize || 
...
void Board::AddConflict(int i, int j, ValueType val) {
    ...
    if (i < 1 || i > boardSize || j < 1 || j > boardSize ||
        val < minValue || val > maxValue)
        throw rangeError("Bad value in AddConflict");
